...
class Elasticsearch
{
    ...
    public function __construct()
    {
        ...
        $host = Yaf_Registry::get('config')->elasticsearch->host;
        $port = Yaf_Registry::get('config')->elasticsearch->port;
    ...
    public function highlight($filedArr, $pre_tags = '<em>', $post_tags = '</em>')
    {
        ...
        foreach ($filedArr as $filedKey => $filedVal) {
            $newFiledArr[$filedVal] = array(
                "require_field_match" => false
            );
        }
        $this->options['highlight'] = array(
            "pre_tags" => array(
                $pre_tags
            ),
            "post_tags" => array(
        ...
        if (! empty($response) && $response['hits']['total'] > 0) {
            ...
            if (! empty($this->options['group'])) {
                ...
                foreach ($response['hits']['hits'] as $key => $val) {
                    ...
                    // 有指定字段数组整理
                    if (! empty($this->options['fields'])) {
                        foreach ($val['fields'] as $field => $value) {
                            $data[$field] = $value[0];
                        }
                        // 提取高亮内容
                        if (! empty($this->options['highlight'])) {
     ...
     *            文档id
     * @param array $data
     *            要修改的数据 必须保证数据字段和添加时全部对应 array('字段1'=>'值1', '字段2'=>'值2' ...)
     * @return bool|array
     */
    ...
    public function delete()
    {
        ...
        if (! empty($response) && $response['_shards']['successful'] > 0) {
            return true;
        } else {
            return false;
        }
